...
class CVCString(CVCExpression):
    ...
    def constant(cls, v, solver):
        ...
        chararray = [CVC4.CVC4String_convertCharToUnsignedInt(c) for c in bytes(v, 'UTF-8')]
    ...
    def getvalue(self):
        ...
        chararray = [CVC4.CVC4String_convertUnsignedIntToChar(c) for c in ce.getConstString().getVec()]
    ...
    def __getitem__(self, item):
        if isinstance(item, slice):
            ...
            self.solver.guards.append(item.start >= CVCInteger.constant(0, self.solver))
            self.solver.guards.append(offset >= CVCInteger.constant(0, self.solver))
            # Adding these forms of guards globally can limit the number of solutions generated
            # but restructuring to avoid reducing the solution space is a significant undertaking.
    ...
    def startswith(self, prefix):
        return CVCExpression(self.em.mkExpr(CVC4.STRING_PREFIX, prefix.cvc_expr, self.cvc_expr), self.solver)
